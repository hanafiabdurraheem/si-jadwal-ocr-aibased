...
<body>
            ...
            <?php foreach ($scheduleItems as $item): ?>
                ...
                <div class="schedule-card <?php echo $isActive ? 'active' : ''; ?>" data-id="<?php echo htmlspecialchars($itemId); ?>">
                    ...
                    <div class="schedule-actions">
                        ...
                        <a class="edit-link" href="index.php?route=jadwal-confirm-edit&schedule_id=<?php echo urlencode($itemId); ?>">
                            Edit
                        </a>
                    </div>
                </div>
  ...
  <script>
    ...
    uploadInput.addEventListener("change", async () => {
        ...
        try {
            const response = await fetch("index.php?route=api-upload&ajax=1", {
                method: "POST",
    ...
    toggles.forEach(toggle => {
        toggle.addEventListener("change", async (event) => {
            ...
            try {
                const response = await fetch("index.php?route=api-set-active-schedule", {
                    method: "POST",
    ...
    renameButtons.forEach(button => {
        button.addEventListener("click", async () => {
            ...
            try {
                const response = await fetch("index.php?route=api-rename-schedule", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
